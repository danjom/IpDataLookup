...
namespace IpAddressDataRetriever.API.Handlers
{
    public static class DistributedWorkersHandler
    {
        ...
        /// <summary>
        /// This methods assigns the works to the workers, in case there is more chunks than workers, then
        /// more then 1 chunk is assigned to each worker in separated requests(1 request per chunk)
        /// </summary>
        /// <param name="workingChunks"></param>
        ...
        public static async Task<JObject> RetrieveDataFromWorkerAsync(List<RequestChunk> workingChunks, string ipOrDomain, int workersCount, int inputType)
        {
            ...
            if(workingChunks?.Count > 0 && !string.IsNullOrWhiteSpace(ipOrDomain) && workersCount > 0 && inputType != InputTypes.Invalid)
            {
